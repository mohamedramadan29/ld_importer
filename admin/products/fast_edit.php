...
if (isset($_POST['edit_cat'])) {
    ...
    if (empty($formerror)) {
        $stmt = $connect->prepare("UPDATE products SET cat_id=?,name=?,slug=?,price=?,sale_price=?,publish=?,product_status_store=?,feature_product=?,as_banner=? WHERE id = ? ");
        $stmt->execute(array($cat_id, $name, $slug, $price, $sale_price, $publish, $product_status_store, $feature_product, $as_banner, $pro_id));
        if ($stmt) {
